...
const Navbar = () => {
  ...
  return (
    <div  style={{ backgroundColor: siteData.headerColor }}>
      <NavbarTop />
      {/* Main navigation container */}
      <nav
        className="flex-no-wrap relative flex w-full  lg:mb-0 items-center justify-between  py-2 shadow-md shadow-black/5
        ...
        <div className="flex w-full flex-wrap items-center justify-between px-3  py-3 mb-0 sm:mb-16 lg:mb-0">
          ...
          >
            ...
            >
              {menu?.length > 0 &&
                menu?.map((item, index) => (
                  <MenuNavbar item={item} lang={lang} key={index} />
                ))}
            </ul>
          </div>
          {/* Right elements */}
